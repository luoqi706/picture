/**
 * SillyTavern 古风桌宠助手 (V8.1)
 * 暂不支持任何二传二改到其他角色卡的行为
 */
/* global Mvu, createChatMessages */
(function () {
    console.log("ST Pet: 脚本开始加载 (V8.1)...");
    let doc = document;
    let win = window;
    ...
    try {
        ...
        console.warn("ST Pet: 无法访问父级文档（跨域限制），使用本地文档", e);
        doc = document;
        win = window;
    }
    const _ = window._ || {
    ...
    const cleanup = () => {
        ...
        targets.forEach(t => {
            try {
                const p = t.getElementById(PET_ID);
                if (p) p.remove();
                const s = t.getElementById(STYLE_ID);
                if (s) s.remove();
            } catch (e) {}
        });
    };
...
信件指向的写信对象并不固定。角色给{{user}}写信和给其他人物写信出现的概率平均分布。每次正式开始写作之前随机生成一个整数，如果是奇数，则本次写作"收信人"是{{user}}，如果是偶数，则本次信件不是写给{{user}}的。
所有角色都会写信给{{user}}，不要默认所有的信件都是写给{{user}}的，角色也可能会写信给其他角色，例如江辞可能会写信给师门或江湖好友，萧胤会给江南巡抚写秘密圣旨、谢危亭可能会写信给同僚商讨政事等等。不要局限信件的写作内容 and 文字风格。也不要局限于我给出的可能性。禁止照抄范例！！
...
【信件本体】: （书信的具体内容）
【信件类型】: （信件的分类，如家书、军情战报、腹稿、草稿、普通信件、随笔、政事往来、奏折、圣旨、情书、请罪书、幻想等等。）
...
1. 生成逻辑：
输入触发：我会为你指定一个分类（医药用具、珍奇小物、实用物品、闺房隐秘）。
单一执行：接收到分类指令后，你仅允许生成属于该分类下的商品，严禁跨类混合，严禁一次性输出多个分类。
数量：单次随机生成 4-8 个商品。

2. 文本输出规范：
严禁任何正文组件外的文字。禁止开场白、系统注释、小剧场及任何元数据说明。
直接呈现商品列表，严格执行下方强制输出格式。
严格参照下方强制输出格式进行商品呈现。一次性生成物品价值不得过于相近，应该有便宜货也有贵重物品。禁止全部生成华而不实的物品，禁止一次性全部生成装饰品。

3. 物品限制：
内容需符合古代常识，不可包含权力象征物（如玉玺）。
“闺房隐秘”分类完全为nsfw内容，涉及情趣用品，性玩具，情趣内衣，相关药物以及其他与nsfw主题相关的商品，需自然生成相关物品。
物品价值需有高低起伏，不得全部生成为贵重物品；不得全部生成为廉价物品。

4. 强制输出格式（逐个商品独立成组）：
【物品类型】: [此处仅填写当前指定的分类名称]
【物品名称】: [2~7字古风名称]
【物品描述】: [古雅具体的细节描写，50字以内]
【使用指南】: [用一段文字说明此物如何使用、在何种情境下可派上用场、使用时有无禁忌或讲究。若为与剧情相关的物品，需暗示其与当前宫闱之事的内在关联；若为珍奇小玩意儿，需说明其玩赏之趣或巧思所在；若为实用物品，需讲明其功效益处与实际操作之法。字数不得超过100字，严格控制字数]
...
禁止出现任何超现实、玄幻、现代或不符合古代宫廷语境的物品。`
    ...
    const customFontCSS = data.settings.fontUrl ? `
        ...
        body, #${PET_ID}, #${PET_ID} * { font-family: 'GF_Custom_Font', "Microsoft YaHei", serif !important; }
    ` : '';
    ...
    style.innerHTML = `
       #${PET_ID} { 
        ...
        .gf-root-container { 
            ...
            bottom: 10%; 
            left: 50%; 
            margin-left: calc(${currentSize.width} / -2);
        ...
        @media screen and (max-width: 768px) {
            .gf-root-container { 
                top: 150px; 
                bottom: auto;
                margin-left: calc((${currentSize.width} * 0.8) / -2);
                transform: scale(0.8); 
            ...
            .gf-chat-window, .gf-dialog, .gf-inspect-box, .gf-theater-window { 
                top: 18% !important; 
                left: 50% !important;
            ...
            .gf-dialog { width: 90vw !important; }
            .gf-theater-window {
            ...
            .gf-extra-chapter { font-size: 22px !important; }
            .gf-theater-body {
            ...
            .gf-menu { left: auto !important; right: 100% !important; margin-left: 0; margin-right: 10px; }
            .gf-menu::before { left: auto; right: -12px; border-right: none; border-left: 12px solid rgba(255, 255, 255, 0.7); }
        }
        .gf-visual { width: 100%; height: 100%; cursor: grab; background: transparent !important; display: flex; align-items: center; justify-content: center; }
        .gf-visual img { max-width: 100%; max-height: 100%; object-fit: contain; pointer-events: none; filter: drop-shadow(0 10px 20px rgba(0,0,0,0.2)); -webkit-user-drag: none; }
        ...
        .gf-chat-body { flex: 1; padding: 20px; overflow-y: auto; display: flex; flex-direction: column; gap: 25px; position: relative; height: auto; scrollbar-width: none; }
        ...
        /* 模板页面 */
        .gf-template-grid { display: grid; grid-template-columns: 1fr; gap: 15px; padding: 10px; }
        .gf-template-card { 
            aspect-ratio: auto; 
            min-height: 120px;
        ...
        .gf-letter-body { padding: 20px; display: none; animation: gfSlideDown 0.4s ease; }
        .gf-letter-content { 
            font-size: 16px; 
            line-height: 2.4; 
            color: #34495e; 
        ...
        .gf-letter-meta { display: grid; grid-template-columns: 1fr 1fr; gap: 15px 12px; font-size: 13px; color: #666; }
        .gf-meta-item { display: flex; flex-direction: column; align-items: flex-start; gap: 4px; }
        .gf-meta-label { 
            color: #b33939; 
            font-weight: 600; 
            opacity: 0.9; 
        ...
        .gf-meta-content { color: #555; line-height: 1.4; }
        ...
        /* 通用弹窗 - 梦幻风格 */
        .gf-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.2); display: none; z-index: 2147483640; backdrop-filter: blur(8px); pointer-events: auto !important; }
        .gf-dialog { 
            position: fixed; 
            top: 50%; 
            left: 50%; 
        ...
        /* 琅琊秘宝 - 分页与动画 (V8.1) */
        .gf-mall-tabs::-webkit-scrollbar { display: none; }
        .gf-mall-tab { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); border-bottom: 2px solid transparent; }
        .gf-mall-tab.active { border-bottom-color: #b33939; color: #b33939 !important; font-weight: bold; transform: translateY(-1px); }
        .gf-mall-page { transition: opacity 0.4s ease, transform 0.4s cubic-bezier(0.4, 0, 0.2, 1); }
        .gf-mall-page.fade-out { opacity: 0; transform: translateX(-20px); }
        .gf-mall-page.fade-in { opacity: 1; transform: translateX(0); }
        @media (max-width: 450px) {
            .gf-dialog { width: 95% !important; padding: 20px !important; }
            .gf-mall-tabs { gap: 10px !important; }
            .gf-mall-tab { font-size: 13px !important; padding: 5px 2px !important; }
            .gf-mall-item { padding: 8px !important; }
            .gf-mall-btn-add { padding: 5px !important; font-size: 10px !important; }
        }
    ...
    root.id = PET_ID;
    const currentImg = data.settings.petMode === 'gif' ? (IMG_GIF + '?t=' + Date.now()) : IMG_STATIC;
    root.innerHTML = `
        ...
        <div class="gf-root-container">
            ...
            <div class="gf-visual" id="gf-visual">
                <img id="gf-img" src="${currentImg}" alt="桌宠" referrerPolicy="no-referrer">
            </div>
    ...
    const container = root.querySelector('.gf-root-container');
    if (data.pos) {
        container.style.left = data.pos.left;
        container.style.top = data.pos.top;
    ...
    const getTavernContext = async () => {
        const getRunnerApi = (name) => {
            try { 
                ...
                if (win.parent && win.parent !== win) {
                    try { if (typeof win.parent[name] === 'function') return win.parent[name]; } catch(e) {}
                }
                if (typeof window[name] === 'function') return window[name];
                if (window.parent && window.parent !== window) {
                    try { if (typeof window.parent[name] === 'function') return window.parent[name]; } catch(e) {}
                }
            } catch(e) {}
            return null;
        };
        ...
        try {
                ...
                try {
                    const getMacroFn = () => {
                        ...
                        for (const name of names) {
                            if (typeof win[name] === 'function') return win[name];
                            if (typeof window[name] === 'function') return window[name];
                            try { if (win.parent && typeof win.parent[name] === 'function') return win.parent[name]; } catch(e) {}
                        }
                        return null;
                    };
                    ...
                    const fn = getMacroFn();
                    if (fn) {
                        const p = await fn('{{persona}}');
                        if (p && p !== '{{persona}}') return p;
                    }

                    // 尝试 SillyTavern.name1 (用户名)
                    let st = win.SillyTavern || window.SillyTavern;
                    if (!st && win.parent && win.parent !== win) {
                        try { st = win.parent.SillyTavern; } catch(e) {}
                    }
            ...
            if (_getCharCardData) {
                ...
                } catch(e) { console.warn("ST Pet: 抓取角色描述失败", e); }
            }
            ...
            if (_getCharWorldbookNames) {
                ...
                } catch(e) { console.warn("ST Pet: 抓取世界书名称失败", e); }
            }
            ...
            if (_getChatHistoryDetail) {
                ...
                } catch(e) { console.warn("ST Pet: 抓取对话历史失败", e); }
            }
            ...
            if (results.persona === "未找到面具设定" || results.chatHistory === "未找到对话历史") {
                let stContext = null;
                try { 
                    const st = win.SillyTavern || window.SillyTavern || (win.parent && win.parent.SillyTavern);
                    stContext = st?.getContext?.(); 
                } catch (e) {}
    ...
    const startInkReview = async () => {
        ...
        if (st) {
            systemPrompt += `
【1. 用户面具设定】：${st.persona}
【2. 角色描述】：${st.description}
【3. 专属设定（江辞背景故事）】：${st.jiangCiEntry}
【4. 世界书定义前】：${st.wiBefore.join('\n')}
【5. 世界书定义后】：${st.wiAfter.join('\n')}
【6. 对话历史 (近五楼)】：${st.chatHistory}
`;
        }
    ...
    const renderMailboxList = () => {
        ...
        chatBody.innerHTML = `
            ...
            <div class="gf-protagonist-list">
                <div class="gf-protagonist-btn" data-name="萧胤" data-cmd="letterxiaoyi">萧胤</div>
        ...
        mailboxLetters.forEach((msg) => {
            ...
            card.innerHTML = `
                ...
                <div class="gf-letter-footer">
                    <div class="gf-letter-meta" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px 12px;">
                        <div class="gf-meta-item"><span class="gf-meta-label">✍ 字迹</span><span class="gf-meta-content">${handwriting}</span></div>
                        <div class="gf-meta-item"><span class="gf-meta-label">📍 位置</span><span class="gf-meta-content">${location}</span></div>
                        <div class="gf-meta-item"><span class="gf-meta-label">📜 状态</span><span class="gf-meta-content">${status}</span></div>
                        <div class="gf-meta-item"><span class="gf-meta-label">🎬 场景</span><span class="gf-meta-content">${scene}</span></div>
                    </div>
                </div>
...
`;
        ...
        if (st) {
            const isLost = userInput.startsWith('letter_lost_');
            systemPrompt += `
【1. 用户面具设定】：${st.persona}
【2. 角色描述】：${st.description}
【3. 专属设定（江辞背景故事）】：${st.jiangCiEntry}
...
`;
            if (!isLost) {
                systemPrompt += `【6. 对话历史 (近五楼)】：${st.chatHistory}\n`;
            }
        }
    ...
    const renderTheaterTemplates = () => {
        ...
        const templates = [
            { title: "春日游，杏花吹满头。陌上谁家年少，足风流。", preview: " 萧胤昔年于霍府花园初遇user", prompt: "现在立刻停止生成正文和原著剧情，进入番外环节。暂停当前剧情为我生成一个番外。番外剧情不计入正文，与主线剧情无关。请以此为开端：那是萧胤尚未登基、身为皇子时的一段往事。当日萧胤因公出巡路经霍府，特来拜会老家主。霍老爷热忱相邀，请他在府内花园暂作小憩。当萧胤步入那片繁花锦簇时，正巧撞见了正在园中独自消遣的{{user}}。请以此初见为核心，细致刻画萧胤当时的惊艳感与两人之间微妙的氛围，严格按照“番外剧场”预设格式输出。" },
            ...
            { title: "蓦然回首，那人却在，灯火阑珊处。", preview: "谢危亭于某个深夜想起user的瞬间", prompt: "现在立刻停止生成正文和原著剧情，进入番外环节。暂停当前剧情为我生成一个番外。番外剧情不计入正文，与主线剧情无关。请以此为开端：夜深人静，丞相府书房内烛火摇曳。谢危亭正襟坐于案前处理堆积如山的公文。或许是夜色太静，又或许是案角的某样物件勾起了回忆，他的思绪不由自主地飘到了{{user}}身上。那是他埋藏极深的情感，在这一刻排山倒海而来。请重点刻画谢危亭细腻且隐忍的心理活动，描述他在孤独权力中心的那一抹温存，严格按照“番外剧场”预设格式输出。" },
            { title: "山野农户，烟火人家", preview: "如果你我兄妹只是小村里最寻常不过的百姓", prompt: "现在立刻停止生成正文和原著剧情，进入番外环节。暂停当前剧情为我生成一个番外。番外剧情不计入正文，与主线剧情无关。遗忘所有背景故事设定，仅保留人物性格和外貌设计，进入平行世界环节，此刻霍砥舟和{{user}}只是最普通不过的寻常百姓人家。环境设定：大景王朝边缘的穷乡僻壤。霍砥舟不再是将军，而是一个背负着‘克亲’名声、沉默寡言的猎户；{{user}}是他唯一的血亲，仅剩的小妹妹。大雪封山，哪怕是砖屋也冻得吓人。霍砥舟在炉子边生火，把唯一的半块烤红薯塞进{{user}}手里，随后上榻抱着妹妹，也就是{{user}}用体温取暖。营造一种禁忌暧昧又日常的氛围。严格禁止将霍砥舟塑造成不讲卫生的邋遢糙汉。严格按照“番外剧场”预设格式输出。" },
            { title: "买单半天发现花的是前男友的钱", preview: "现pa：酒吧买酒，江辞的亲情卡扣款怎么一直在响", prompt: "现在立刻停止生成正文和原著剧情，进入番外环节。暂停当前剧情为我生成一个番外。番外剧情不计入正文，与主线剧情无关。遗忘所有背景故事设定，仅保留人物性格和外貌设计，衣着打扮跟随时代背景合理化变动，进入平行世界环节：故事背景是现代社会，{{user}}与江辞是前任的关系，已经分手数月。这天{{user}}和朋友来到酒吧喝酒玩乐，刷支付宝买酒，没注意花成了绑定着江辞账户的亲情卡，刷出去一笔巨款，而江辞看到了扣款信息。简略扩写提示内容后继续输出后面的故事，严格按照“番外剧场”预设格式输出。" },
            { title: "🔞OOC预警：新婚夜团圆", preview: "我是说，如果萧胤没有生在皇家，也许他真的能长成一个君子？", prompt: "现在立刻停止生成正文和原著剧情，进入番外环节。暂停当前剧情为我生成一个番外。番外剧情不计入正文，与主线剧情无关。遗忘所有背景故事设定，仅保留人物性格和外貌设计，生成if线剧情，不是皇族的京城某家大族大公子萧胤，没有经历过被人下菜碟没有摸爬滚打命悬一线，没经历过权力浸染和勾心斗角，发展成了一个真君子。生成他和未婚妻{{user}}的洞房花烛夜，不避讳nsfw剧情。严格按照“番外剧场”预设格式输出。" },
            { title: "🔞情蛊血脉焚枯时，何人与我共赴极乐？", preview: "没嫁人的妹妹中了合欢散", prompt: "现在立刻停止生成正文和原著剧情，进入番外环节。暂停当前剧情为我生成一个番外。番外剧情不计入正文，与主线剧情无关。遗忘所有背景故事设定，仅保留人物性格和外貌设计，生成if线剧情，{{user}}没有入宫也没有嫁人的背景下，意外中了烈性春毒合欢散，必须要与人交合才能解毒。此刻她身边只有自己的哥哥霍砥舟，两人正在酒楼的某间隔音厢房里，短时间不会有人来打扰。简单扩写背景故事，随后针对后续的剧情发展进行写作，本篇为nsfw剧情。严格按照“番外剧场”预设格式输出。" }
        ];
...
`;
    ...
    const renderMall = async () => {
        ...
        const _Mvu = win.Mvu || window.Mvu;
        const categories = ["医药用具", "珍奇小物", "实用物品", "闺房隐秘"];
        
        // 保持购物车在当前弹窗生命周期内全局
        if (!win._mallCart) win._mallCart = [];
        if (!win._currentMallTab) win._currentMallTab = categories[0];

        const getMoney = () => {
            if (typeof _Mvu === 'undefined') return { total: 0, data: null };
            let targetData = null;
            if (_Mvu.getMvuData) {
                const types = ['character', 'global', 'chat'];
                for (let t of types) {
                    const d = _Mvu.getMvuData({ type: t });
                    if (d && d.stat_data && _.has(d.stat_data, '主角背包')) {
                        targetData = d;
                        break;
                    }
                }
                if (!targetData) targetData = _Mvu.getMvuData({ type: 'message', message_id: 'latest' });
            } else if (_Mvu.get) {
                targetData = _Mvu.get('SillyTavern');
            }
        ...
        const renderItemsHTML = () => {
            const filteredItems = data.mallItems.filter(i => i.category === win._currentMallTab);
            if (filteredItems.length === 0) {
                return `
                    <div style="flex:1; display:flex; flex-direction:column; justify-content:center; align-items:center; color:#999; font-style:italic; min-height:200px;">
                        <div style="font-size:40px; margin-bottom:10px; opacity:0.3;">🏮</div>
                        <div>此柜台尚无存货，请江辞补货...</div>
                    </div>
                `;
            }
            return `
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; animation: gfSlideDown 0.3s ease;">
                    ${filteredItems.map((item) => {
                        // 寻找在原始数据中的索引以便操作
                        const originalIdx = data.mallItems.indexOf(item);
                        return `
                            <div class="gf-mall-item" style="background:#fff; border:1px solid #d6c4a1; border-radius:8px; padding:10px; display:flex; flex-direction:column; justify-content:space-between; position:relative; box-shadow:0 2px 5px rgba(0,0,0,0.02);">
                                <div>
                                    <div style="font-weight:bold; font-size:13px; color:#333; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${item.name}</div>
                                    <div style="color:#b33939; font-size:12px; font-weight:bold; margin:3px 0;">${item.cost}文</div>
                                    <div style="font-size:11px; color:#666; line-height:1.4; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; height:30px;">${item.desc}</div>
                                </div>
                                <button class="gf-mall-btn-add" data-idx="${originalIdx}" style="background:#b33939; color:white; border:none; border-radius:4px; padding:6px; font-size:11px; cursor:pointer; margin-top:8px; transition:opacity 0.2s;">放入篮中</button>
                                <div class="gf-mall-item-inspect" data-idx="${originalIdx}" style="position:absolute; top:5px; right:5px; font-size:10px; color:#999; cursor:pointer; padding:2px;">🔍</div>
                            </div>
                        `;
                    }).join('')}
                </div>
        ...
        const renderShopUI = () => {
            ...
            content.innerHTML = `
                <div class="gf-mall-container" style="display:flex; flex-direction:column; height:100%; position:relative; overflow:hidden;">
                    <!-- 盘缠头部 -->
                    <div style="text-align:center; border-bottom:1px solid #d6c4a1; padding-bottom:8px; margin-bottom:10px;">
                        <div style="font-size:10px; color:#888; margin-bottom:2px;">身上盘缠</div>
                        <b style="font-size:16px; color:#b33939;">${totalCopper} <span style="font-size:11px;">文</span></b>
                    </div>

                    <!-- 分类页眉 -->
                    <div class="gf-mall-tabs" style="display:flex; justify-content:space-between; margin-bottom:12px; border-bottom:1px solid rgba(214, 196, 161, 0.3); padding-bottom:2px; overflow-x:auto; scrollbar-width:none;">
                        ${categories.map(cat => `
                            <div class="gf-mall-tab ${win._currentMallTab === cat ? 'active' : ''}" data-cat="${cat}" style="padding:6px 4px; font-size:14px; cursor:pointer; color:#888; white-space:nowrap; flex:1; text-align:center;">
                                ${cat}
                            </div>
                        `).join('')}
                    </div>
                    ...
                    <!-- 商品主体 -->
                    <div id="mall-body-wrapper" style="flex:1; overflow:hidden; position:relative; min-height:280px;">
                        <div id="mall-body" class="gf-mall-page fade-in" style="height:100%; overflow-y:auto; padding-right:4px;">
                            ${renderItemsHTML()}
                        </div>
                    </div>

                    <!-- 购物车抽屉 -->
                    <div id="mall-cart-drawer" style="display:none; margin-top:10px; background:#f9f7f0; border-radius:12px; padding:12px; border:1px solid #d6c4a1; box-shadow:0 -5px 15px rgba(0,0,0,0.03);">
                        <div style="font-size:11px; font-weight:bold; color:#555; margin-bottom:8px; display:flex; justify-content:space-between;">
                            <span>待购清单</span>
                            <span id="btn-clear-cart" style="color:#999; font-weight:normal; cursor:pointer;">清空</span>
                        </div>
                        <div id="mall-cart-list" style="max-height:120px; overflow-y:auto; scrollbar-width:none;"></div>
                    </div>

                    <!-- 底部统计与操作 -->
                    <div style="margin-top:12px; border-top:1px solid #b33939; padding-top:10px;">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                            <span id="btn-mall-toggle-cart" style="font-size:12px; color:#666; cursor:pointer; display:flex; align-items:center; gap:4px;">
                                🛒 篮中物 (<span id="mall-cart-count">0</span>)
                            </span>
                            <b style="color:#b33939; font-size:15px;">合计：<span id="mall-cart-total">0</span> 文</b>
                        </div>
                        <div style="display:flex; gap:10px;">
                            <button id="btn-mall-refresh" style="flex:1; background:#95a5a6; color:white; border:none; padding:10px; border-radius:8px; font-weight:bold; cursor:pointer; font-size:12px; transition:background 0.3s;">请求补货</button>
                            <button id="btn-mall-submit" style="flex:2; background:#27ae60; color:white; border:none; padding:10px; border-radius:8px; font-weight:bold; cursor:pointer; font-size:12px; transition:background 0.3s;">确认结账</button>
                        </div>
                    </div>
                    
                    <!-- 加载遮罩 -->
                    <div id="mall-loading" style="display:none; position:absolute; top:0; left:0; width:100%; height:100%; background:rgba(253,250,242,0.98); z-index:100; flex-direction:column; justify-content:center; align-items:center; border-radius:12px;">
                        <div class="gf-loading-spinner"></div>
                        <div id="mall-loading-text" style="color:#b33939; font-size:13px; margin-top:15px; letter-spacing:1px; text-align:center; padding:0 20px;">江辞正在搜罗奇珍异宝...</div>
                    </div>
                </div>
            `;
            ...
            const mallBody = doc.getElementById('mall-body');

            const updateCartUI = () => {
                cartList.innerHTML = '';
                let total = 0;
                let count = 0;
                win._mallCart.forEach((item, i) => {
                    total += item.cost * item.count;
                    count += item.count;
                    const row = doc.createElement('div');
                    row.style = "display:flex; justify-content:space-between; align-items:center; font-size:12px; margin-bottom:6px; background:#fff; padding:8px; border-radius:8px; border:1px solid #eee;";
                    row.innerHTML = `
                        <span style="font-weight:bold; flex:1; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; margin-right:10px;">${item.name}</span>
                        <div style="display:flex; align-items:center; gap:8px;">
                            <button class="mall-cart-adj" data-idx="${i}" data-delta="-1" style="width:22px; height:22px; border:1px solid #ddd; background:#f9f9f9; border-radius:4px; cursor:pointer; display:flex; align-items:center; justify-content:center;">-</button>
                            <span style="min-width:15px; text-align:center;">${item.count}</span>
                            <button class="mall-cart-adj" data-idx="${i}" data-delta="1" style="width:22px; height:22px; border:1px solid #ddd; background:#f9f9f9; border-radius:4px; cursor:pointer; display:flex; align-items:center; justify-content:center;">+</button>
                            <button class="mall-cart-rem" data-idx="${i}" style="margin-left:5px; color:#e74c3c; background:none; border:none; cursor:pointer; font-size:14px;">×</button>
                        </div>
                    `;
                ...
                if (win._mallCart.length === 0) cartDrawer.style.display = 'none';

                doc.querySelectorAll('.mall-cart-adj').forEach(btn => {
                    btn.onclick = () => {
                        const idx = parseInt(btn.dataset.idx);
                        const delta = parseInt(btn.dataset.delta);
                        win._mallCart[idx].count += delta;
                        if (win._mallCart[idx].count <= 0) win._mallCart.splice(idx, 1);
                        updateCartUI();
                    };
                });
                doc.querySelectorAll('.mall-cart-rem').forEach(btn => {
                    btn.onclick = () => {
                        win._mallCart.splice(parseInt(btn.dataset.idx), 1);
                        updateCartUI();
                    };
                });
            };
            ...
            const bindItemEvents = () => {
                doc.querySelectorAll('.gf-mall-btn-add').forEach(btn => {
                    btn.onclick = () => {
                        ...
                        const exist = win._mallCart.find(c => c.name === item.name);
                        if (exist) exist.count++;
                        else win._mallCart.push({ ...item, count: 1 });
                        updateCartUI();
                        showBubble(`已将【${item.name}】放入篮中。`);
                    };
                });
            ...
            // 切换分类逻辑
            doc.querySelectorAll('.gf-mall-tab').forEach(tab => {
                tab.onclick = async () => {
                    const newCat = tab.dataset.cat;
                    if (newCat === win._currentMallTab) return;
                    
                    mallBody.classList.remove('fade-in');
                    mallBody.classList.add('fade-out');
                    
                    await new Promise(r => setTimeout(r, 300));
                    
                    win._currentMallTab = newCat;
                    doc.querySelectorAll('.gf-mall-tab').forEach(t => t.classList.remove('active'));
                    tab.classList.add('active');
                    
                    mallBody.innerHTML = renderItemsHTML();
                    mallBody.classList.remove('fade-out');
                    mallBody.classList.add('fade-in');
            ...
            doc.getElementById('btn-clear-cart').onclick = () => {
                win._mallCart = [];
                updateCartUI();
            };
            ...
            doc.getElementById('btn-mall-refresh').onclick = async () => {
                ...
                doc.getElementById('mall-loading-text').innerText = `江辞正在搜罗【${win._currentMallTab}】...`;
                try {
                    await triggerMallGeneration(win._currentMallTab);
                    mallBody.innerHTML = renderItemsHTML();
                    loading.style.display = 'none';
                    showBubble(`${win._currentMallTab}已补货。`);
                    bindItemEvents();
                } catch (e) {
                    loading.style.display = 'none';
                    alert("补货失败，请检查通灵配置。");
            ...
            doc.getElementById('btn-mall-submit').onclick = async () => {
                const totalCost = win._mallCart.reduce((s, i) => s + (i.cost * i.count), 0);
                ...
                if (typeof _Mvu !== 'undefined') {
                    if (_Mvu.replaceMvuData) {
                        await _Mvu.replaceMvuData(freshData, { type: targetData.type || 'message', message_id: targetData.message_id });
                    } else if (_Mvu.set) {
                        _Mvu.set('SillyTavern', freshData);
                    }
                }
    ...
    const triggerMallGeneration = async (category) => {
        if (!data.config.apiKey) throw new Error("API Key missing");
        const st = await getTavernContext();
        let systemPrompt = `你现在是琅琊秘宝的掌柜江辞。\n${data.presets.mallRules}`;
        const prompt = `请立刻为我生成一批新的【${category || '随机'}】分类下的琅琊秘宝商品，必须按照【琅琊秘宝规范】的内容进行写作。一次性生成4-8个商品。`;
        ...
        blocks.forEach(block => {
            ...
            const itemCategory = extract('物品类型') || category;
            ...
            if (name && desc) {
                items.push({ name, desc, guide, cost: cost || 1000, category: itemCategory });
            }
        });
        ...
        if (items.length > 0) {
            // 局部更新：仅替换当前分类的商品
            if (category) {
                data.mallItems = data.mallItems.filter(i => i.category !== category).concat(items);
            } else {
    ...
    async function openSettingsDialog() {
        ...
        content.innerHTML = `
            ...
            <div class="gf-section-box">
                <label class="gf-label">危险操作</label>
                <button class="gf-btn" style="background:#e74c3c; width:100%;" id="btn-reset-data">初始化全部内容 (清空所有本地数据)</button>
            </div>
            <div class="gf-section-box">
        ...
        doc.getElementById('btn-debug-st').onclick = () => {
            ...
            try {
                const stObj = win.SillyTavern || (win.parent && win.parent.SillyTavern);
                console.log("SillyTavern Object:", stObj);
                const ctx = (stObj || window.SillyTavern)?.getContext?.();
                console.log("Context:", ctx);
            } catch(e) { 
                console.log("Debug Access Error:", e); 
            }
            alert("已将酒馆核心对象输出至浏览器控制台 (F12)，请查看。");
        };

        // 初始化按钮
        doc.getElementById('btn-reset-data').onclick = () => {
            const confirmed = win.confirm ? win.confirm("确定要彻底初始化脚本吗？这相当于卸载重装：将删除所有本地配置、信件历史和商店数据，并自动刷新页面以加载仓库最新预设，此操作无法恢复。") : window.confirm("确定要彻底初始化脚本吗？这相当于卸载重装：将删除所有本地配置、信件历史和商店数据，并自动刷新页面以加载仓库最新预设，此操作无法恢复。");
            if (confirmed) {
        ...
        doc.getElementById('btn-save-settings').onclick = () => {
            const setFont = doc.getElementById('set-font');
            const setMode = doc.getElementById('set-mode');
            const setSize = doc.getElementById('set-size');
            const setUserPersona = doc.getElementById('set-user-persona');
            
            if (setFont) data.settings.fontUrl = setFont.value;
            if (setMode) data.settings.petMode = setMode.value;
            if (setSize) data.settings.petSize = setSize.value;
            if (setUserPersona) data.presets.userPersonaOverride = setUserPersona.value;
        ...
        doc.getElementById('btn-reset-data').onclick = () => {
            if (win.confirm("确定要初始化全部内容吗？这将删除所有本地设置、预设和历史记录，且不可恢复。")) {
                localStorage.removeItem(storageKey);
                location.reload();
            }
        };
    ...
    // 拖拽与点击逻辑 (V8.0 增强版：兼容移动端)
    let isDragging = false, hasMoved = false, startPos = { x: 0, y: 0 }, startOffset = { x: 0, y: 0 };
    ...
    const onMove = (e) => {
        ...
        const newX = startOffset.x + clientX - startPos.x;
        const newY = startOffset.y + clientY - startPos.y;
