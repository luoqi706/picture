...
禁止出现任何超现实、玄幻、现代或不符合古代宫廷语境的物品。`
    ...
    style.innerHTML = `
       #${PET_ID} { 
        ...
        /* 1. 元数据网格容器：保持 2x2 布局不变，但增加行间距 */
.gf-letter-meta { 
    display: grid; 
    grid-template-columns: 1fr 1fr; 
    gap: 15px 12px; /* 增加上下行之间的距离 */
    font-size: 13px; 
    color: #888; 
}
...
.gf-meta-item { 
    display: flex; 
    flex-direction: column; /* 👈 关键：强制让内容显示在标题下方 */
...
.gf-meta-label::after {
    ...
    width: 100%;
    height: 8px; /* 高光粗细 */
    background: rgba(179, 57, 57, 0.15); /* 高光颜色，使用了主色调的淡化版 */
    z-index: -1; /* 确保在文字下方 */
    transform: skewX(-15deg); /* 带一点倾斜，更有手绘感 */
    border-radius: 2px;
}
...
.gf-meta-value {
    color: #555;
    padding-left: 4px; /* 与标题稍微错开，增加层次感 */
    line-height: 1.4;
...
`;
    ...
    <div class="gf-letter-footer">
        <div class="gf-letter-meta">
            <div class="gf-meta-item">
                <span class="gf-meta-label">✍ 字迹</span>
                <span class="gf-meta-value">${handwriting}</span>
            </div>
            <div class="gf-meta-item">
                <span class="gf-meta-label">📍 位置</span>
                <span class="gf-meta-value">${location}</span>
            </div>
            <div class="gf-meta-item">
                <span class="gf-meta-label">📜 状态</span>
                <span class="gf-meta-value">${status}</span>
            </div>
            <div class="gf-meta-item">
                <span class="gf-meta-label">🎬 场景</span>
                <span class="gf-meta-value">${scene}</span>
            </div>
        </div>
    </div>
...
`;
    ...
    const renderTheaterTemplates = () => {
        ...
        const templates = [
            { title: "春日游，杏花吹满头。陌上谁家年少，足风流。", preview: " 萧弈昔年于霍府花园初遇user", prompt: "现在立刻停止生成正文和原著剧情，进入番外环节。暂停当前剧情为我生成一个番外。番外剧情不计入正文，与主线剧情无关。请以此为开端：那是萧弈尚未登基、身为皇子时的一段往事。当日萧弈因公出巡路经霍府，特来拜会老家主。霍老爷热忱相邀，请他在府内花园暂作小憩。当萧弈步入那片繁花锦簇时，正巧撞见了正在园中独自消遣的{{user}}。请以此初见为核心，细致刻画萧弈当时的惊艳感与两人之间微妙的氛围，简略扩写提示内容后继续输出后面的故事，严格按照“番外剧场”预设格式输出。" },
            ...
            { title: "金风玉露", preview: "莫名意外变小，正好撞上萧既白入宫请安", prompt: "现在立刻停止生成正文和原著剧情，进入番外环节。暂停当前剧情为我生成一个番外。番外剧情不计入正文，与主线剧情无关。请以此为开端：这是一个充满荒诞色彩的深宫日。{{user}}竟因意外异象突然变回了孩童模样。宫中侍女乱作一团、六神无主之际，萧既白正巧步入殿内请安。面对眼前这个小小的、熟悉又陌生的身影，一向冷静的萧既白流露出了难得的无措与柔情。请以此冲突为点撰写番外，简略扩写提示内容后继续输出后面的故事，严格按照“番外剧场”预设格式输出。" },
            { title: "蓦然回首，那人却在，灯火阑珊处。", preview: "谢危亭于某个深夜想起user的瞬间", prompt: "现在立刻停止生成正文和原著剧情，进入番外环节。暂停当前剧情为我生成一个番外。番外剧情不计入正文，与主线剧情无关。请以此为开端：夜深人静，丞相府书房内烛火摇曳。谢危亭正襟危坐于案前处理堆积如山的公文。或许是夜色太静，又或许是案角的某样物件勾起了回忆，他的思绪不由自主地飘到了{{user}}身上。那是他埋藏极深的情感，在这一刻排山倒海而来。请重点刻画谢危亭细腻且隐忍的心理活动，描述他在孤独权力中心的那一抹温存，简略扩写提示内容后继续输出后面的故事，严格按照“番外剧场”预设格式输出。" },
