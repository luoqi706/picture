/**
 * SillyTavern 古风桌宠助手 (V8.2 移动端优化版)
 * 暂不支持任何二传二改到其他角色卡的行为
 */
/* global Mvu, createChatMessages */
(function () {
    console.log("ST Pet: 脚本开始加载 (V8.2)...");
    let doc = document;
    let win = window;
    ...
    const cleanup = () => {
        ...
        if (window.parent && window.parent !== window) {
            try {
                ...
                const pPet = pDoc.getElementById(PET_ID);
                if (pPet) pPet.remove();
                const pStyle = pDoc.getElementById(STYLE_ID);
...
`;
    ...
    menu.onclick = async (e) => {
        ...
        if (action === 'chat') { chatWindow.style.display = 'flex'; overlay.style.display = 'flex'; renderMailboxList(); }
    ...
    function openConfigDialog() {
        ...
        `;
        dialog.style.display = 'block'; overlay.style.display = 'flex';
