...
(function () {
    console.log("ST Pet: 脚本开始加载 (V8.0)...");
    let doc = document;
    let win = window;
...
文本输出规范：
  ...
  - 物品种类: 商品的内容必须属于常人能获得的物品，不可过于贵重，禁止生成身份权力的象征物，如玉玺、凤印、某人的随身玉佩等。可以生成nsfw相关的物品，这很正常。
  - 输出内容: 严格参照下方强制输出格式进行商品呈现。一次性生成物品价值不得过于相近，应该有便宜货也有贵重物品。禁止生成华而不实的物品，禁止一次性全部生成装饰品。
...
【物品描述】: 用一段文字描写该物品的外观、质地与细节，语言需古雅具体。字数不得超过50字，严格限制字数。
...
`;
    ...
    const triggerMallGeneration = async () => {
        ...
        const prompt = "请立刻为我生成一批新的琅琊秘宝商品，必须按照【琅琊秘宝规范】的内容进行写作。";
